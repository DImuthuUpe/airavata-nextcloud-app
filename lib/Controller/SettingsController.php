...
class SettingsController extends Controller
{
    ...
    public function saveSettings($airavata_keycloak_url, $airavata_apiserver_url, $airavata_realm, $airavata_keycloak_superuser_name,
                                 $airavata_keycloak_superuser_password, $airavata_keycloak_auth_granttype, $airavata_keycloak_client_id, $client_id, $client_secret, $introspect_url, $token_type)
    {
        try {
	    ...
	    $this->config->setAppValue($this->appName, 'client_secret', $client_secret);
	    $this->config->setAppValue($this->appName, 'introspect_url', $introspect_url);
	    $this->config->setAppValue($this->appName, 'token_type', $token_type);
            return array(
                'code' => 200,
